...
|---|---|---|---|---|
| Workload A: Daily User Retention & DAU Composite Index Test | < 500 ms | < 3000 ms (NFR-2) | **0.67 ms** | **PASS (< 500ms Target)** |
| Workload B: Discipline & Professor Rating Score Rollup | < 500 ms | < 3000 ms (NFR-2) | **3.9 ms** | **PASS (< 500ms Target)** |
| Workload C: User Session Duration & Engagement Analysis | < 500 ms | < 3000 ms (NFR-2) | **1.1 ms** | **PASS (< 500ms Target)** |
...
```text
Index Scan using idx_fct_daily_activity_date_user on fct_daily_user_activity (actual time=0.038..0.038 rows=0.00 loops=1)
...
#### PostgreSQL Raw Plan Output
```text
GroupAggregate (actual time=0.816..0.817 rows=1.00 loops=1)
  Group Key: c.codigo_disciplina, c.nome_disciplina
  ->  Sort (actual time=0.769..0.772 rows=9.00 loops=1)
        Sort Key: c.codigo_disciplina, c.nome_disciplina
        Sort Method: quicksort  Memory: 25kB
        ->  Nested Loop (actual time=0.335..0.366 rows=9.00 loops=1)
              ->  Bitmap Heap Scan on fct_ratings r (actual time=0.296..0.299 rows=9.00 loops=1)
                    Recheck Cond: (date_sk = 20260115)
                    Heap Blocks: exact=1
                    ->  Bitmap Index Scan on idx_fct_ratings_date_sk (actual time=0.010..0.010 rows=9.00 loops=1)
...
#### PostgreSQL Raw Plan Output
```text
GroupAggregate (actual time=0.136..0.150 rows=19.00 loops=1)
  Group Key: u.user_id
  ->  Sort (actual time=0.130..0.133 rows=19.00 loops=1)
        Sort Key: u.user_id
        Sort Method: quicksort  Memory: 25kB
        ->  Nested Loop (actual time=0.042..0.096 rows=19.00 loops=1)
              ->  Bitmap Heap Scan on fct_sessions s (actual time=0.031..0.033 rows=19.00 loops=1)
                    Recheck Cond: (session_start_date_sk = 20260115)
                    Heap Blocks: exact=1
                    ->  Bitmap Index Scan on idx_fct_sessions_start_date_sk (actual time=0.009..0.009 rows=19.00 loops=1)
